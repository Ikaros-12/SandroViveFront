...
@Component({
  selector: 'app-devolucionesyreembolsos',
  templateUrl: './devolucionesyreembolsos.component.html',
  styleUrls: ['./devolucionesyreembolsos.component.css']
})
export class DevolucionesyreembolsosComponent {
  ...
  DevolucionesPorPeriodo(): void{    
    ...
    if(this.filtro.fecha_desde && this.filtro.fecha_hasta){
      this.informesService.getDevolucionesPorPeriodo(this.filtro.fecha_desde, this.filtro.fecha_hasta).subscribe({
        next: (response) => {
          ...
          // }
          console.log("Lista de Productos Menos Vendidos por Periodo:");
  ...
  ExportarExcel(): void {
    ...
    const data = this.devoluciones_por_periodo.map(devoluciones => ({
      fecha: devoluciones.fecha_creacion,
      nro_pedido: devoluciones.pedido_id,
      cantidad_articulos: devoluciones.total_cantidad,
      monto_total: devoluciones.total,
      observaciones: devoluciones.observaciones,
    }));

    this.exportarExcelService.ExportarAExcel(data, headers, 'devoluciones_por_periodo');
  }
